...
def conv_block_1(inputs, filters, activation, batch_norm):
...
def conv_block_2(inputs, filters, activation, batch_norm, drop):
    hidden = MaxPooling2D(pool_size=(2, 2))(inputs)
    hidden = Dropout(drop)(hidden) if drop else hidden
    outputs = conv_block_1(hidden, filters, activation, batch_norm)
    return outputs


def conv_block_3(inputs, layer, filters, activation, batch_norm, drop):
...
def get_model(image_shape, filters, depth, inc_rate, activation, drop, batch_norm):
    ...
    layers = []
    hidden = conv_block_1(inputs, filters, activation, batch_norm)
    layers.append(hidden)
    for index in range(depth):
        filters = filters*inc_rate
        hidden = conv_block_2(hidden, filters, activation, batch_norm, drop)
        if index != depth-1:
            layers.append(hidden)
    for index in range(depth):
        filters = filters//inc_rate
        hidden = conv_block_3(hidden, layers[-1-index], filters, activation, batch_norm, drop)
